...
#define SONYIMX188_FLASH_TRIGGER_GAIN	0x60
...
#define SONYIMX188_APERTURE_FACTOR   240 //F2.4
#define SONYIMX188_EQUIVALENT_FOCUS  24  //24mm

static camera_capability sonyimx188_cap[] = {
	{V4L2_CID_FOCAL_LENGTH, 152},//1.52mm
};
...
#define SONYIMX188_MIDDLE_FRAMERATE		10
...
static int sonyimx188_enum_framesizes(struct v4l2_frmsizeenum *framesizes)
{
	...
	framesizes->discrete.width = sonyimx188_framesizes[this_max_index].width;
	framesizes->discrete.height = sonyimx188_framesizes[this_max_index].height;
...
static int sonyimx188_init_reg(void)
{
	...
	if(scamera_id_value)
	{
		...
		strncpy(sonyimx188_sensor.info.name, "sonyimx188_byd",sizeof(sonyimx188_sensor.info.name));
	...
	else
	{
		...
		strncpy(sonyimx188_sensor.info.name, "sonyimx188_sunny",sizeof(sonyimx188_sensor.info.name));
...
static int sonyimx188_update_flip(u16 width, u16 height)
{
	...
	u8 old_flip = sonyimx188_sensor.old_flip;
	print_debug("Enter %s  ", __func__);
...
static int sonyimx188_get_capability(u32 id, u32 *value)
{
	int i;
	for (i = 0; i < sizeof(sonyimx188_cap) / sizeof(sonyimx188_cap[0]); ++i) {
		if (id == sonyimx188_cap[i].id) {
			*value = sonyimx188_cap[i].value;
			break;
		}
	}
	return 0;
...
static u32 sonyimx188_get_override_param(camera_override_type_t type)
{
	...
	case OVERRIDE_FLASH_TRIGGER_GAIN:
		ret_val = SONYIMX188_FLASH_TRIGGER_GAIN;
		break;
...
static int sonyimx188_get_sensor_aperture()
{
	return SONYIMX188_APERTURE_FACTOR;
}

static int sonyimx188_get_equivalent_focus()
{
	return SONYIMX188_EQUIVALENT_FOCUS;
}
...
static void sonyimx188_set_default(void)
{
	...
	sonyimx188_sensor.get_capability = sonyimx188_get_capability;
	...
	strncpy(sonyimx188_sensor.info.name,"sonyimx188", sizeof(sonyimx188_sensor.info.name));
	...
	sonyimx188_sensor.get_sensor_aperture = sonyimx188_get_sensor_aperture;
	sonyimx188_sensor.get_equivalent_focus = sonyimx188_get_equivalent_focus;
	...
	sonyimx188_sensor.old_flip = 0;
	sonyimx188_sensor.lcd_compensation_supported = 1;
}
