...
static int dload_temp_firmware(struct rmi_device *rmi_dev, struct reflash_data *data)
{
	...
	retval = rmi_read_block(rmi_dev, test_addr, test_configid, 4);
	if (retval < 0)
		dev_err(&rmi_dev->dev, "Failed to read test_configid (code %d).\n", retval);
	...
	if(!temp_fw_entry){
		dev_err(&rmi_dev->dev, "temp_fw_entry is null\n");
		return -1;
	}
